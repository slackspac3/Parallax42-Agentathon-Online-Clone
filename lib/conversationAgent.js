...
function visibleQuestionFromHistory(history = []) {
  if (!Array.isArray(history)) return '';
  const turns = history.filter((turn) => turn && typeof turn === 'object');
  const assistantQuestion = turns.slice().reverse().find((turn) => (
    turn.role === 'assistant'
    && cleanText(turn.displayedQuestion)
    && !/^what do you need reviewed\??$/i.test(cleanText(turn.displayedQuestion))
  ))?.displayedQuestion;
  if (assistantQuestion) return cleanText(assistantQuestion);
  return cleanText(turns.slice().reverse().find((turn) => turn.role === 'user' && turn.answeringQuestion)?.answeringQuestion || '');
}
...
function activeQuestionForTurn(input = {}, existingDraft = {}) {
  const eventType = cleanText(input.eventType || existingDraft.currentEventType || '');
  const visibleQuestion = visibleQuestionFromHistory(input.history);
  const explicitQuestion = cleanText(input.activeQuestion);
  if (eventType === 'user_answer' && visibleQuestion) return visibleQuestion;
  return explicitQuestion || cleanText(existingDraft.activeQuestion || (Array.isArray(existingDraft.questions) ? existingDraft.questions[0] : '')) || visibleQuestion;
}
...
function processConversation(input = {}, options = {}) {
  ...
  const activeQuestion = activeQuestionForTurn(input, existingDraft);
